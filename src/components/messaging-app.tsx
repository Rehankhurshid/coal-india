...
export function MessagingApp() {
  ...
  const [searchQuery, setSearchQuery] = useState('') // For actual filtering (debounced)

  // Debounce search input to avoid constant filtering
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      setSearchQuery(searchInput)
    }, 300) // 300ms delay for conversation search

    return () => clearTimeout(timeoutId)
  }, [searchInput])

  // Using mock WebSocket to avoid parsing errors - replace with useWebSocket for production
  ...
  return (
    <ErrorBoundary
      onError={(err, errorInfo) => {
        ...
        console.error("Messaging app error:", err, errorInfo)
      }}
    >

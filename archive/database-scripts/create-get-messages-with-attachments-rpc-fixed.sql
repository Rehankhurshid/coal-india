...
DROP FUNCTION IF EXISTS get_group_messages_with_names(integer, integer, integer);

-- Create the improved function with correct data types matching the actual database schema
...
BEGIN
  ...
  SELECT 
    ...
    m.reply_to_id,
    m.edit_count,
    COALESCE(e.name, 'Unknown User')::text as sender_name,
    r.content as reply_content,
    COALESCE(re.name, 'Unknown User')::text as reply_sender_name,
    COALESCE(
      (
        SELECT json_agg(
          json_build_object(
...
-- Sample usage:
-- SELECT * FROM get_group_messages_with_names(1, 50, 0);
